...
Function write_callback CDecl _
    ...
    If quit=1 Then myidata=0:Return 0 'stop
    Return bytes
End Function
...
Function httppostcurl(ByRef host As String,ByRef path As String)As String'Integer
...
Var url=host
    ...
    If Left(url,4)<>"http" Then
    	If InStr(LCase(url),"wiki")>0 Then
    		url="https://"+url
    	Else 	
    		url="http://"+url
    	EndIf
    ...
    If useragent="" Then useragent="chatbot_chung"+Str(Int(Rnd*1000))
    ...
    '?"retcode="+Str(retcode)
    If retcode<>0 Then
    	guinotice "error curl="+Str(retcode)+"  "+url'curl_easy_strerror(retcode))
    	myidata=0
    EndIf
    'myztext+=Chr(0)
...
'guinotice Str(myidata)
   ...
   recvtext=myztext
   'guinotice left(recvtext,400)
Return recvtext'myidata
End Function 
/'
