...
Sub testwiki()
   randomize 
   Var webtext=httppostcurl(hostname,path)
   print
...
Dim Shared As Integer twiki=1,wikinword=1,forcewiki=0
...
auxvar=0
forcewiki=0
If LCase(Left(intext22,5))="wiki " Then
	forcewiki=1:intext22=Mid(intext22,6)
EndIf
randomwiki=max(0.03,min(1,randomwiki+0.03))
If Rnd<(0.25*randomwiki+forcewiki) Then 
 parse(Trim(intext22)," ")
 wikinword=iparse
